...
beq x3, x0, length_verified # If null terminator, exit loop
addi x2, x2, 1            # Increment digit count
addi x1, x1, 1            # Move to the next character
j check_length

length_verified:
li x4, 16                 # Expected length
...
beq x5,x0, validate    # If null terminator is found, exit loop

addi x5, x5, -48           # Convert ASCII to integer

li x31,2
# Check if position is odd or even
rem x6, x2, x31             # Remainder to check if position is odd
